...
app.post('/api/logbook/sync', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Apply pending client-side deletions (tombstones queued while offline)
    ...
    if (Array.isArray(deletions) && deletions.length) {
      await supabase
        ...
        .eq('user_id', userId)
        .in('id', deletions);
      console.log(`[Logbook sync] Applied ${deletions.length} pending deletions for user=${userId}`);
    }
...
// GET /api/logbook  — return active entries + tombstone deleted_ids
app.get('/api/logbook', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Tombstones — deleted in last 90 days (pruning older ones)
    ...
    const entries    = (activeData  || []).map(r => r.data).filter(Boolean);
    const deleted_ids = (deletedData || []).map(r => r.id);
    res.json({ entries, deleted_ids, count: entries.length });
  } catch (e) {
...
// ─────────────────────────────────────────────────────────────────
// LOGBOOK TOMBSTONE MIGRATION (run once in Supabase SQL Editor):
//
//   ALTER TABLE logbook_entries
//     ADD COLUMN IF NOT EXISTS is_deleted  BOOLEAN     DEFAULT false,
//     ADD COLUMN IF NOT EXISTS deleted_at  TIMESTAMPTZ;
//
//   CREATE INDEX IF NOT EXISTS idx_logbook_deleted
//     ON logbook_entries (user_id, is_deleted, deleted_at);
// ─────────────────────────────────────────────────────────────────

// DELETE /api/logbook/:id — soft-delete (tombstone)
...
// POST /api/logbook/bulk-delete — soft-delete multiple entries atomically
app.post('/api/logbook/bulk-delete', requireAuth, async (req, res) => {
  if (!supabase) return res.status(500).json({ error: 'Base de datos no configurada' });
  const { ids } = req.body;
  ...
  try {
    const { error } = await supabase
      ...
      .in('id', ids);
    if (error) throw error;
    console.log(`[Logbook bulk-delete] user=${req.user.id} soft-deleted=${ids.length}`);
    res.json({ ok: true, deleted: ids.length });
  } catch (e) {
    console.error('[Logbook bulk-delete]', e.message);
    res.status(500).json({ error: e.message });
  }
});
  ...
  return new Promise((resolve, reject) => {
    ...
    parser.on('pdfParser_dataReady', data => {
      try {
        ...
        // ── Home base detection ──────────────────────────────────────────────────────
        // Priority 1: explicit "(BCN CP)" / "(BCN FO)" / "(BCN OPL)" / etc. in the header
        // Covers all Vueling rank codes: CP, FO, SO, OPL, HOD, HOF, SFO, PNT, OPT…
        let home_base = null;
        const baseMatch = ft.match(/\(\s*([A-Z]{3})\s+(?:CP|FO|SO|OPL|HOD|HOF|SFO|PNT|OPT|[A-Z]{2,4})\s*\)/);
        if (baseMatch) {
          home_base = baseMatch[1];
          console.log('[RosterPDF] Base detectada en cabecera:', home_base);
        }
        // Priority 2: mode of the first-DEP per operating day (strip leading *)
        if (!home_base) {
          const _sortedE = allEntries.filter(e => e.type === 'flight' && e.dep)
            .slice().sort((a,b)=>a.date.localeCompare(b.date)||
              (parseInt((a.std||'00:00').split(':')[0])*60+parseInt((a.std||'00:00').split(':')[1])) -
              (parseInt((b.std||'00:00').split(':')[0])*60+parseInt((b.std||'00:00').split(':')[1])));
          const _dayFirstDep = {};
          for (const e of _sortedE) {
            const dep = (e.dep||'').replace(/^\*/,'').toUpperCase();
            if (dep && !_dayFirstDep[e.date]) _dayFirstDep[e.date] = dep;
          }
          const _depCount = {};
          for (const dep of Object.values(_dayFirstDep)) _depCount[dep] = (_depCount[dep]||0) + 1;
          let maxCnt = 0;
          for (const [dep, cnt] of Object.entries(_depCount)) {
            if (cnt > maxCnt) { maxCnt = cnt; home_base = dep; }
          }
          if (home_base) console.log('[RosterPDF] Base inferida por moda DEP:', home_base, '('+maxCnt+' días)');
        }
        ...
        // ── Crossover flight fix ────────────────────────────────────────────────────
        // Vueling PDF places the ARR and STA of overnight flights in the ARRIVAL DAY's
        // column (not the departure day). The arrival day column starts with ↓ followed
        // by ARR (IATA) and STA (SHH:MM). Without this fix the flight contributes 0 min
        // to block hours.
        // Example: TUN→BCN departs Jun22 at 23:55, arrives Jun23 at 01:45. The STA
        // "S01:45" lives in the Jun23 column → the Jun22 entry has no sta → 0 block.
        const _ARROW_RE = /^[↓→]$/;
        for (const e of allEntries) {
          if (e.type !== 'flight') continue;
          if (e.sta || e.sta_actual) continue;          // already resolved
          const stdStr = e.std || e.std_actual;
          if (!stdStr) continue;
          const stdMins = parseInt(stdStr.split(':')[0]) * 60 + parseInt(stdStr.split(':')[1]);
          if (stdMins < 1200) continue;                 // only departures after 20:00
          const dayNum = parseInt(e.date.slice(-2));
          const nextDayNum = dayNum + 1;
          if (!dayColsMap[nextDayNum]) continue;
          // Read next day's sorted tokens
          const nextTokens = words
            .filter(w => w.y >= 13 && w.y <= 60 && Math.abs(w.x - dayColsMap[nextDayNum]) <= colW * 0.65)
            .sort((a, b) => a.y - b.y)
            .map(w => w.text);
          // First token must be the down-arrow crossover indicator
          if (!nextTokens.length || !_ARROW_RE.test(nextTokens[0])) continue;
          // Scan for ARR (IATA) and STA (SHH:MM)
          let foundArr = null, foundSta = null;
          for (let j = 1; j < nextTokens.length; j++) {
            const tok = nextTokens[j];
            if (!foundArr && /^[A-Z]{3}$/.test(tok))  foundArr = tok;
            if (!foundSta && /^[ASE]\d{1,2}:\d{2}$/.test(tok)) {
              const timeVal = tok.slice(1);
              if (tok[0] === 'A') e.sta_actual = timeVal; else e.sta = timeVal;
              foundSta = timeVal;
            }
            if (foundArr && foundSta) break;
          }
          if (foundArr && !e.arr) e.arr = foundArr;
          if (foundSta) {
            console.log(`[RosterPDF] Crossover fix: ${e.date} VY${e.flightNum} `
              + `${e.dep}→${e.arr} STD ${stdStr} STA ${foundSta} (from day ${nextDayNum} column)`);
          }
        }

        const nF = allEntries.filter(e=>e.type==='flight').length;

        // ── Extract D (duty) total from bottom row ──
        // We compute pdf_block_min from the parsed entries (same logic as front-end blockMin)
        // because the B row is unreliable: day-22→23 crossover nights cause (a) a corrupted
        // clock-time on day 22 ("22:10" = 22 h 10 m instead of ~3 h 35 m) and (b) an arrival
        // time on day 23 ("01:45") that looks like a valid block figure but isn't.
        // The entry-based approach is exact and immune to both failure modes.
        const TIME_RE = /^\d{1,2}:\d{2}$/;
        const MAX_DUTY_DAY_MIN = 1200;  // 20 h — conservative FTL ceiling
        const _t2m = s => { const p=(s||'').split(':'); return (+p[0]||0)*60+(+p[1]||0); };
        let pdf_block_min = 0;
        for (const e of allEntries) {
          if (e.type !== 'flight') continue;
          const isPos = (e.dep && e.dep.startsWith('*')) || e.positioning === true;
          if (isPos) continue;
          const effStd = e.std_actual || e.std;
          const effSta = e.sta_actual || e.sta;
          if (!effStd || !effSta) continue;
          let m = _t2m(effSta) - _t2m(effStd);
          if (m < 0) m += 1440;    // midnight crossing
          if (m > 0 && m < 960) pdf_block_min += m;  // sanity: no single leg > 16 h
        }
        console.log('[RosterPDF] pdf_block_min (entry-based): '
          + Math.floor(pdf_block_min/60)+'h'+(pdf_block_min%60)+'m');

        // D row: duty totals (still reliable from the PDF)
        const dWord = words.find(w => w.text === 'D');
        let pdf_duty_min = 0;
        if (dWord) {
          const dY = dWord.y, tolerance = 0.8;
          words.filter(w => Math.abs(w.y - dY) <= tolerance && TIME_RE.test(w.text))
            .forEach(w => {
              ...
              if(mins < MAX_DUTY_DAY_MIN) {
                ...
                console.warn('[RosterPDF] ⚠ Ignorando valor D sospechoso: "'+w.text+'" ('+mins+'min)');
              }
            });
        }

        resolve({ month: monthStr, entries: allEntries, pdf_block_min, pdf_duty_min, home_base });
